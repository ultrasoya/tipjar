...
interface DonationListModalContentProps {
  onClose: () => void;
  donations: Donation[];
...
export function DonationListModalContent({
  onClose,
  donations,
  totalDonations,
  onLoadMore,
          ...
          {donations?.length === 0 ? (
            ...
            <div className={styles.donationsContainer}>
              {donations?.map((donation) => (
                <motion.div
                  key={donation.id || `${donation.donor}-${donation.timestamp}`}
